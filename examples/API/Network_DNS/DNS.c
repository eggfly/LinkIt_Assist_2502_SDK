...
/* 
  This exmple gets the IP address of labs.mediatek.com
...
{
    vm_log_info("dns result:%d.%d.%d.%d", (result->address[0]) & 0xFF, ((result->address[0]) & 0xFF00)>>8, ((result->address[0]) & 0xFF0000)>>16,
...
static void get_host(VM_TIMER_ID_NON_PRECISE timer_id, void* user_data)
{
    ...
    g_handle = vm_dns_get_host_by_name(VM_BEARER_DATA_ACCOUNT_TYPE_GPRS_CUSTOMIZED_APN, HOST, &result, get_host_callback, NULL);
