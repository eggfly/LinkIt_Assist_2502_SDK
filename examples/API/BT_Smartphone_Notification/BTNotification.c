...
static void app_btcm_cb(VMUINT evt, void * param, void * user_data){
    ...
    switch (evt){
        case VM_BT_CM_EVENT_ACTIVATE:{
            ...
            vm_log_debug("BTCM VM_BT_CM_EVENT_ACTIVATE hdl[%d] rst[%d]", active->handle, active->result);
            ...
            vm_log_debug("BTCM vm_bt_cm_get_host_device_info[%s][0x%02x:%02x:%02x:%02x:%02x:%02x]", dev_info.name,
        ...
        case VM_BT_CM_EVENT_INQUIRY_IND:{
            ...
            vm_log_debug("BTCM VM_BT_CM_EVENT_INQUIRY_IND hdl[%d] num[%d]", ind->handle, ind->discovered_device_number);
            break;
        }
...
VMINT btns_test_notification_callback(vm_bt_ns_notification_data_t* parameter_ptr, void* user_data){
    VMUINT8 content[100];
    ...
    /* Shows the content */
    vm_chset_ucs2_to_ascii((VMSTR)content, 100, (VMWSTR)parameter_ptr->message->sender);
    vm_log_debug("refresh cb sender = %s", (VMSTR)content);
    /* Shows the sender's number */
    memset(content, 0, 100);
    vm_chset_ucs2_to_ascii((VMSTR)content, 100, (VMWSTR)parameter_ptr->message->sender_number);
    vm_log_debug("refresh cb sender number = %s", (VMSTR)content);
    /* Shows the content sender's content */
    memset(content, 0, 100);
    vm_chset_ucs2_to_ascii((VMSTR)content, 100, (VMWSTR)parameter_ptr->message->content);
    vm_log_debug("refresh cb sender content = %s", (VMSTR)content);
    memset(content, 0, 100);
    vm_chset_ucs2_to_ascii((VMSTR)content, 100, (VMWSTR)parameter_ptr->message->title);
    vm_log_debug("refresh cb sender title = %s", (VMSTR)content);
    vm_log_debug(" missed_call_count = %d", parameter_ptr->message->missed_call_count);
    vm_log_debug("refresh cb sender source = %d", parameter_ptr->message->source);
...
VMINT btns_test_connection_status_change_callback(vm_bt_ns_connection_status_t* parameter_ptr, void* user_data){
...
void vm_main(void){
    ...
    i = vm_bt_ns_register_notification_callback(btns_test_notification_callback, NULL);
    vm_log_debug("refresh_callback handle = %d", i);

    /* Registers the BT connection status change callback */
    i = vm_bt_ns_register_connection_status_change_callback(btns_test_connection_status_change_callback, NULL);
